...
import { SubstationDto, SubstationInfoDto, SubstationKeyDefectDto, SubstationListDto, SubstationNoteDto, SubstationSelectOptionDto } from '#substation/dtos/index'
...
export default class SubstationsController {
  ...
  async getSubstationKeyDefect({ params, response }: HttpContext) {
    const substationParam = params as Params
    const data = await SubstationService.findById(substationParam.id)

    return response.status(200).json(new SubstationKeyDefectDto(data))
  }
