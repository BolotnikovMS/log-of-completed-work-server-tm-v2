...
export default class SubstationService {
  static async getSubstations(
    ...
    const typeKpIds = normalizeToNumberArray(filters?.typeKp)
    const headControllerIds = normalizeToNumberArray(filters?.headController)
    ...
    const substations = await Substation.query()
      ...
      .if(filters?.channelType || filters?.channelCategory, (query) => {
        query.whereHas('channels', query => {
          if (filters?.channelType) query.where('channelTypeId', '=', filters?.channelType)
          if (filters?.channelCategory) query.where('channelCategoryId', '=', filters?.channelCategory)
        })
      })
      .if(typeKpIds?.length || headControllerIds?.length, (query) => {
        query.whereHas('telemechanics_device', (query) => {
          if (typeKpIds?.length) query.whereIn('typeKpId', typeKpIds)
          if (headControllerIds?.length) query.whereIn('headControllerId', headControllerIds)
        })
      })
